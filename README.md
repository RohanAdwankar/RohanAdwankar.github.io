...
Files under `static/` are copied into `dist/` as they are. `static/js/three/` is three.js 0.170.0 (`three.module.min.js` and the four addons the map posts use), which the map posts under `static/places/` load through an import map.
