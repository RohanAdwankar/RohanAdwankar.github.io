...
The map posts under `static/places/` ship their 3D scenes as glTF files built by `scenes/build_scenes.py`, which needs Blender's Python module: `uv run --python 3.13 --with bpy --with pillow scenes/build_scenes.py vienna-1913`. It downloads the Kenney kits it assembles from into `.cache/`, which git ignores.
